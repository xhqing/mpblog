...
相应的分布函数为
$$
P\left(X\leq x\right) = F\left(x\right) = \int_{-\infty}^xf\left(t\right)dt=\frac{1}{\sqrt{2\pi}\sigma}\int_{-\infty}^x\exp\left(-\frac{\left(t-\mu\right)^2}{2\sigma^2}\right)dt
$$
这里首先回忆一下概率密度函数是如何被定义出来的，形如$F\left(x\right)=\int_{-\infty}^xf\left(t\right)dt$的分布函数中的$f\left(t\right)=\frac{d}{dx}F\left(x\right)$被定义为概率密度函数，因此要求变换后的随机变量的概率密度函数因先求其形如$F\left(x\right)$的分布函数。
...
&= \int_{-\infty}^{\sigma x'+\mu}f\left(t\right)dt\\
&= \frac{1}{\sqrt{2\pi}\sigma}\int_{-\infty}^{\sigma x'+\mu}\exp\left(-\frac{\left(t-\mu\right)^2}{2\sigma^2}\right)dt\\
&= \frac{1}{\sqrt{2\pi}\sigma}\int_{-\infty}^{x'}\exp\left(-\frac{u^2}{2}\right)\sigma du\\
&= \int_{-\infty}^{x'}\frac{1}{\sqrt{2\pi}}\exp\left(-\frac{u^2}{2}\right)du\\
&= G\left(x'\right)
\end{aligned}
$$
所以随机变量$X'$的概率密度函数为
$$
g\left(x'\right) = \frac{d}{dx'}G\left(x'\right) = \frac{d}{dx'}\int_{-\infty}^{x'}\frac{1}{\sqrt{2\pi}}\exp\left(-\frac{u^2}{2}\right)du=\frac{1}{\sqrt{2\pi}}\exp\left(-\frac{x'^2}{2}\right)
$$
...
分布函数为
$$
P\left(X\leq x\right) = F\left(x\right) = \int_{-\infty}^x f\left(t\right)dt = 
\begin{cases}
0, &x\leq a\\
\int_a^x\frac{1}{b-a}dt, &a < x < b\\
1, &x \geq b
\end{cases}
$$
...
&= \begin{cases}
0, &\sigma x'+\mu \leq a\\
\int_a^{\sigma x'+\mu}\frac{1}{b-a}dt, &a < \sigma x'+\mu < b\\
1, &\sigma x'+\mu \geq b
\end{cases}\\
&= \begin{cases}
0, &x' \leq \frac{a-\mu}{\sigma}\\
\int_{\frac{a-\mu}{\sigma}}^{x'}\frac{1}{b-a}\sigma du, &\frac{a-\mu}{\sigma}<x'<\frac{b-\mu}{\sigma}\\
1, &x'\geq \frac{b-\mu}{\sigma}
\end{cases}\\
&= \begin{cases}
0, &x' \leq \frac{a-\mu}{\sigma}\\
\int_{\frac{a-\mu}{\sigma}}^{x'}\frac{1}{\frac{b-\mu}{\sigma}-\frac{a-\mu}{\sigma}} du, &\frac{a-\mu}{\sigma}<x'<\frac{b-\mu}{\sigma}\\
1, &x'\geq \frac{b-\mu}{\sigma}
\end{cases}\\
&= G\left(x'\right)
\end{aligned}
...
因此$X'$的概率密度函数为
$$
g\left(x'\right) = \frac{d}{dx'}G\left(x'\right) = \begin{cases}
...
注意这里Box-Cox变换后是近似服从正态分布而不是严格服从正态分布，因此没办法对变换后的变量严格推导出服从正态分布的概率密度函数，因此要用另外一种方法来检验变换后的数据是否近似服从正态分布，这种方法统称为正态性检验。

那么如何对数据进行正态性检验呢？

正态性检验是一类检验数据是否近似服从正态分布的方法统称，常用的正态性检验方法有偏度-峰度检验法、正态概率纸法、夏皮罗一威尔克检验法(Shapiro-Wilktest)，科尔莫戈罗夫检验法等。

在此仅尝试采用最常用的偏度-峰度检验法来从数学公式推导的角度进行理解。

那么首先要知道偏度-峰度检验法的数学原理。
...
分布函数为
$$
P\left(Z\leq z\right) = F\left(z\right) = \int_{-\infty}^z\frac{1}{\sqrt{2\pi}}\exp\left(-\frac{t^2}{2}\right)dt
$$
...
&= \int_{-\infty}^{y^\frac{1}{3}}\frac{1}{\sqrt{2\pi}}\exp\left(-\frac{t^2}{2}\right)dt\\
&= \int_{-\infty}^y\frac{1}{\sqrt{2\pi}}\exp(-\frac{u^{\frac{2}{3}}}{2})du^{\frac{1}{3}}\\
&= \int_{-\infty}^y\frac{1}{\sqrt{2\pi}}\exp(-\frac{u^{\frac{2}{3}}}{2})\frac{1}{3}u^{-\frac{2}{3}}du\\
&= \frac{1}{3\sqrt{2\pi}}\int_{-\infty}^{y}\exp\left(-\frac{u^{\frac{2}{3}}}{2}\right)u^{-\frac{2}{3}}du\\
&= G\left(y\right)
\end{aligned}
$$
因此$Y$的概率密度函数为
$$
g\left(y\right) = \frac{d}{dy}G\left(y\right)= \frac{1}{3\sqrt{2\pi}}\exp\left(-\frac{y^{\frac{2}{3}}}{2}\right)y^{-\frac{2}{3}}
$$
...
&= \int_{-\infty}^{+\infty}yg\left(y\right)dy\\
&= \frac{1}{3\sqrt{2\pi}}\int_{-\infty}^{+\infty}\exp\left(-\frac{y^{\frac{2}{3}}}{2}\right)y^{\frac{1}{3}}dy\\
&=0
\end{aligned}
$$
上式中$\exp\left(-\frac{y^{\frac{2}{3}}}{2}\right)y^{\frac{1}{3}}$为关于原点对称的奇函数，因此积分结果为0，所以最终可以得出服从正态分布的随机变量$X$的偏度为$0$。
...
&= \int_{-\infty}^{y^{\frac{1}{4}}}\frac{1}{\sqrt{2\pi}}\exp\left(-\frac{t^2}{2}\right)dt-\int_{-\infty}^{-y^{\frac{1}{4}}}\frac{1}{\sqrt{2\pi}}\exp\left(-\frac{t^2}{2}\right)dt\\
&=\int_{-\infty}^0\frac{1}{\sqrt{2\pi}}\exp\left(-\frac{t^2}{2}\right)dt + \int_{0}^{y^{\frac{1}{4}}}\frac{1}{\sqrt{2\pi}}\exp\left(-\frac{t^2}{2}\right)dt-\int_{-\infty}^{-y^{\frac{1}{4}}}\frac{1}{\sqrt{2\pi}}\exp\left(-\frac{t^2}{2}\right)dt\\
&=2\int_{0}^{y^{\frac{1}{4}}}\frac{1}{\sqrt{2\pi}}\exp\left(-\frac{t^2}{2}\right)dt\\
&=2\int_{0}^y\frac{1}{\sqrt{2\pi}}\exp\left(-\frac{u^{\frac{1}{2}}}{2}\right)du^{\frac{1}{4}}\\
&= 2\int_0^y\frac{1}{\sqrt{2\pi}}\exp\left(-\frac{u^{\frac{1}{2}}}{2}\right)\frac{1}{4}u^{-\frac{3}{4}}du\\
&= \frac{1}{2\sqrt{2\pi}}\int_0^{y}\exp\left(-\frac{u^{\frac{1}{2}}}{2}\right)u^{-\frac{3}{4}}du\\
&= G\left(y\right)
\end{aligned}
$$
因此$Y$的概率密度函数为
$$
g\left(y\right) = \frac{d}{dy}G\left(y\right) = \frac{1}{2\sqrt{2\pi}}\exp\left(-\frac{y^{\frac{1}{2}}}{2}\right)y^{-\frac{3}{4}}
$$
所以峰度$k$可以计算如下
$$
\begin{aligned}
...
&= \mathbb E\left[Y\right] \\
&= \int_{0}^{+\infty}yg\left(y\right)dy\\
&= \frac{1}{2\sqrt{2\pi}}\int_{0}^{+\infty}\exp\left(-\frac{y^{\frac{1}{2}}}{2}\right)y^{\frac{1}{4}}dy\\
